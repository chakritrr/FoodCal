...
const AppDataSource = new DataSource({
  ...
  username: process.env.DB_USERNAME || 'foodcal_user',
  password: process.env.DB_PASSWORD || 'foodcal_password',
  database: process.env.DB_DATABASE || 'foodcal',
  entities: [FoodEntity],
  synchronize: true,
});

const foodsData: FoodEntity[] = [
...
async function seed() {
  ...
  const repo = AppDataSource.getRepository(FoodEntity);

  for (const food of foodsData) {
    const existing = await repo.findOneBy({ id: food.id });
    if (!existing) {
